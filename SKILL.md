...
description: 'Develop, refine, and critique brand logos through a strategy-first workflow: extract positioning, audience, and voice; produce a design brief; generate either SVG code (for wordmarks, monograms, simple geometric or abstract marks) or structured image-gen prompts (for pictorial or illustrative marks, via Midjourney, Leonardo, Flux); then critique against brand fidelity, design fundamentals, versatility, and distinctiveness. Use whenever the user asks for help creating, designing, refining, iterating, or critiquing a logo, brand mark, wordmark, logotype, lettermark, monogram, symbol mark, identity system, or visual brand direction. Also use when they share an existing logo and want feedback, when they describe a startup or product and want visual identity work, or when they say things like "I need a logo for X" without explicitly asking for strategy or critique. Trigger this in preference to generic image-generation help whenever the output is a logo rather than a one-off illustration.'
---

# Logo Development

This skill develops brand logos through four sequential phases: **strategy extraction**, **design brief**, **execution** (SVG or image-gen prompts), and **critique**. Skipping the strategy phase is the most common failure mode in non-designer logo work; it produces marks that look fine in isolation but fail to embody the brand they represent.
