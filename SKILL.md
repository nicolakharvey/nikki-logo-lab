...
description: 'Develop, refine, and critique brand logos through a strategy-first workflow: extract positioning, audience, and voice; produce a design brief; execute via SVG code (wordmarks, monograms, simple geometric or abstract marks), structured image-gen prompts (pictorial or illustrative marks, via Midjourney, Leonardo, Flux), or systematic variation exploration (single-lever direction-finding when the visual direction is still open); then critique against brand fidelity, design fundamentals, versatility, and distinctiveness. Use whenever the user asks for help creating, designing, refining, iterating, critiquing, or exploring directions for a logo, brand mark, wordmark, logotype, lettermark, monogram, symbol mark, identity system, or visual brand direction. Also use when they share an existing logo and want feedback, when they describe a startup or product and want visual identity work, when they say "show me options" or "I''m not sure which direction yet", or when they say things like "I need a logo for X" without explicitly asking for strategy or critique. Trigger this in preference to generic image-generation help whenever the output is a logo rather than a one-off illustration.'
---
